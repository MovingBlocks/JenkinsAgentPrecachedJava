    ...
    stages {
        ...
        stage('Java 8') {
            steps {
                ...
                container('kaniko') {
                    sh '''
                        /kaniko/executor -f ./Dockerfile -c $(pwd) --single-snapshot --reproducible --destination=terasology/jenkins-precached-agent:$DOCKER_TAG-jdk8 --build-arg JDKVERSION=jdk8 --cleanup
                    '''
                }
                }
            }
        ...
        stage('Java 11') {
            steps {
                container('kaniko') {
                    sh '''
                        /kaniko/executor -f ./Dockerfile -c $(pwd) --single-snapshot --reproducible --destination=terasology/jenkins-precached-agent:$DOCKER_TAG-jdk11 --build-arg JDKVERSION=jdk11 --cleanup
                    '''
                }
            }
        }
